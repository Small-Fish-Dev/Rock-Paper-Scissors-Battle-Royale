...
public partial class Icon : Label
{
    ...
    public float PixelSize => 40f / UI.Zoom * ScaleToScreen;
    ...
    public void PlaySound()
    {
        ...
        Sound.Play($"sounds/{Type.ToString().ToLower()}.sound");
    }
    ...
    public void OnTick()
    {
        ...
        foreach (var predator in Icon.All[Predator])
        {
            ...
            {
                velocity = (currentPosition - predatorPosition).Normal * Time.Delta * (40f / UI.Zoom) * ScaleToScreen;
                break;
            }
        }
        ...
        foreach (var ally in Icon.All[Type.Value])
        {
            ...
            {
                pushOffset = (currentPosition - allyPosition).Normal * Time.Delta * (15f / UI.Zoom) * ScaleToScreen;
                break;
            }
        }
        ...
        if (Chasing != null)
        {
            velocity += (chasingPosition - currentPosition).Normal * Time.Delta * (50f / UI.Zoom) * ScaleToScreen;
        ...
        // Apply new position
        var jiggle = new Vector2(Game.Random.Float(-1, 1), Game.Random.Float(-1, 1)) * (0.5f / UI.Zoom) * Time.Delta * 60f * ScaleToScreen;
        PixelPosition += velocity.Clamp(0.5f / UI.Zoom * Time.Delta * -60f, 0.5f / UI.Zoom * Time.Delta * 60f) + jiggle + pushOffset;
    }
}
